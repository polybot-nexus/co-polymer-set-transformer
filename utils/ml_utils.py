...
def tecan_read(filename):
    ...
    file_name = (csv_filepath) #pd.read_csv('polybot_app/demo_files/from_tecan/Loop-3-20240402-normalized.csv')  #
    ...
    file_name.to_csv('polybot_app/demo_files/from_tecan/Loop-3-20240402-normalized_scaled.csv', index=None)
    return csv_filepath
...
def smile_to_dft(smile):
  dictionary , _= get_dft_descriptors_dictionary('polybot_app/ml_loop/dft_descriptors_ECPs.csv')
  return dictionary[smile]

def dft_descr(smiles):
  dictionary , descriptor_names= get_dft_descriptors_dictionary('polybot_app/ml_loop/dft_descriptors_ECPs.csv')
  bits = []
  for smile in smiles:
    try:
...
def select_next_exp_ml(dataframe, selected_color, uncertainties, iteration): 
    ...
    if iteration == 0 or iteration ==1:
       EI_values = expectedImprovement(dataframe['delta_e'].values, uncertainties, ybest=0, epsilon=1)
       #upperConfidenceBound(dataframe['delta_e'].values, uncertainties, epsilon=0.01)
    else: 
